...
6. ~~**Un seul timer en cours par personne**~~ — **révisée le 14/09/2026 après usage réel (migration `0020`)** : plusieurs timers en parallèle. Démarrer n'arrête plus rien, il ajoute ; chaque timer a sa carte sur Aujourd'hui (client, type, durée, note, Corriger, Arrêter), la pastille d'en-tête les compte et les arrête d'un tap chacun. Garde-fou : 4 simultanés au plus par personne dans un espace, tenu par un trigger avec verrou (au-delà, « Arrête-en un d'abord » — plus, c'est un oubli). Ni chevauchement interdit ni avertissement : deux timers sur le même créneau sont un choix, la journée peut compter plus d'heures que l'horloge. Le timer vit côté serveur : il survit à la fermeture du téléphone et s'affiche sur tous les appareils.
   **Corriger sans supprimer** : un timer en cours se recale (heure de début) ou s'arrête à une durée choisie ; une entrée terminée se corrige en jour, début, fin et durée (liés, pas de 15) avec sa note, depuis Aujourd'hui comme depuis le détail Par client.
...
- Banc `qa:pulsar` : isolation entre deux organisations, outil coupé → tout disparaît, l'index du timer unique refuse un second timer (depuis `0020` : deux timers acceptés, le cinquième refusé), la contrainte d'arrondi refuse 20 minutes, `restrict` refuse la suppression d'un client avec entrées.
